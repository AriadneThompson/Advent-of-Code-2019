...
for stretch in wire1:
    length = int(stretch[1:])
    
    if stretch[0] == "R":
        for i in range(x, x + length):
            wire_coords[(i, y)] = steps
            steps += 1
...
for stretch in wire2:
    length = int(stretch[1:])
    
    if stretch[0] == "R":
        for i in range(x, x + length):
            if (i, y) in wire_coords.keys():
                intersections[(i, y)] = wire_coords[(i, y)] + steps
            steps += 1
        ...
        for i in range(y, y + length):
            if (x, i) in wire_coords.keys():
                intersections[(x, i)] = wire_coords[(x, i)] + steps
            steps += 1
